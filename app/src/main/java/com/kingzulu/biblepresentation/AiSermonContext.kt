...
object AiSermonContext {
 ...
 fun decide(alternatives:List<String>):ScriptureSpeechDecision{
  for(raw in alternatives){
   ...
   if(isExplicitNavigation(text)){
    AiScriptureEngine.resolveSpeech(listOf(raw))?.let{resolved->
     ScriptureReadingSession.currentState()?.let{
      Regex("\\b(?:go|jump|take me|back)?\\s*(?:back )?(?:to )?verse ([0-9]{1,3})\\b").find(text)?.groupValues?.get(1)?.toIntOrNull()?.let{n->ScriptureReadingSession.goTo(n)?.let{return activate(it,ScriptureSpeechIntent.NAVIGATION,.99)}}
      if(Regex("\\b(next verse|continue to the next verse|go forward one verse)\\b").containsMatchIn(text))ScriptureReadingSession.next()?.let{return activate(it,ScriptureSpeechIntent.NAVIGATION,.99)}
      if(Regex("\\b(previous verse|go back one verse|take me back one verse)\\b").containsMatchIn(text))ScriptureReadingSession.previous()?.let{return activate(it,ScriptureSpeechIntent.NAVIGATION,.99)}
     }
     return activate(resolved,ScriptureSpeechIntent.NAVIGATION,.99)
    }
   }
   ...
   if(refs.size>1||isListLanguage(text)){
    val ordered=reorderByIntent(text,refs);synchronized(queued){queued.clear();queued.addAll(ordered)}
    resolveQueueSelection(text)?.let{return activate(it,ScriptureSpeechIntent.DIRECT_READ,.96)}
    return ScriptureSpeechDecision(ScriptureSpeechIntent.LISTING,queue=ordered,confidence=.96)
   }
 ...
 private fun isExplicitNavigation(t:String)=listOf("next verse","previous verse","repeat that","repeat verse","next chapter","previous chapter","go back one verse","take me back one verse","jump to verse","take me to verse","go to verse","back to verse").any(t::contains)||Regex("\\b(?:go|jump|take me) (?:back )?(?:to )?verse \\d+\\b").containsMatchIn(t)
