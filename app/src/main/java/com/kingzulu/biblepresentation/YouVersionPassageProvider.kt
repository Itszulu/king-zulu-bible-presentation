...
/** Network provider for Bible versions authorised for the King Zulu YouVersion App Key.
 * The key is supplied at build/runtime and is never stored in this source file.
 */
object YouVersionPassageProvider {
    ...
    private val usfm = mapOf(
        ...
        "nehemiah" to "NEH","esther" to "EST","job" to "JOB","psalms" to "PSA","psalm" to "PSA","proverbs" to "PRO",
        ...
        "zechariah" to "ZEC","malachi" to "MAL","matthew" to "MAT","mark" to "MRK","luke" to "LUK","john" to "JHN",
        "acts" to "ACT","romans" to "ROM","1 corinthians" to "1CO","2 corinthians" to "2CO","galatians" to "GAL",
        "ephesians" to "EPH","philippians" to "PHP","colossians" to "COL","1 thessalonians" to "1TH","2 thessalonians" to "2TH",
        "1 timothy" to "1TI","2 timothy" to "2TI","titus" to "TIT","philemon" to "PHM","hebrews" to "HEB","james" to "JAS",
        "1 peter" to "1PE","2 peter" to "2PE","1 john" to "1JN","2 john" to "2JN","3 john" to "3JN","jude" to "JUD","revelation" to "REV"
    )
    ...
    fun get(context: Context, r: BibleReference, translation: BibleTranslation, appKey: String): Result<Verse> {
        ...
        if (appKey.isBlank()) return Result.failure(IllegalStateException("YouVersion App Key is not configured"))
        ...
        return runCatching {
            ...
            try {
                ...
                if (status !in 200..299) throw IllegalStateException("YouVersion request failed ($status)")
                val root = JSONObject(conn.inputStream.bufferedReader().use { it.readText() })
                val data = root.optJSONObject("data") ?: root
                val text = data.optString("content").trim()
                if (text.isBlank()) throw IllegalStateException("No passage text returned")
                val verse = Verse(r, text, translation.abbreviation)
                memoryCache[cacheKey] = verse
                writeDisk(context, cacheKey, verse)
                verse
    ...
    private fun readDisk(context: Context, key: String): Verse? {
        ...
        return runCatching {
            ...
            val r = BibleReference(j.getString("book"), j.getInt("chapter"), j.getInt("start"), j.optInt("end").takeIf { it > 0 })
            Verse(r, j.getString("text"), j.getString("translation"))
        }.getOrNull()
    }
    private fun writeDisk(context: Context, key: String, verse: Verse) {
        val j = JSONObject().put("book", verse.reference.book).put("chapter", verse.reference.chapter)
            .put("start", verse.reference.verseStart ?: 0).put("end", verse.reference.verseEnd ?: 0)
            .put("text", verse.text).put("translation", verse.translation)
