...
@Composable fun KingZuluApp(){
 ...
 var theme by remember{mutableStateOf(LocalBackgroundStore.loadSelected(context))}
 val preview=if(previewKind.isBlank())null else PresentationSlide(previewReference,previewText,previewTranslation,previewKind);val live=if(liveKind.isBlank())null else PresentationSlide(liveReference,liveText,liveTranslation,liveKind)
 fun setPreview(s:PresentationSlide?){previewReference=s?.reference.orEmpty();previewText=s?.text.orEmpty();previewTranslation=s?.translation.orEmpty();previewKind=s?.kind.orEmpty()};fun setLive(s:PresentationSlide?){liveReference=s?.reference.orEmpty();liveText=s?.text.orEmpty();liveTranslation=s?.translation.orEmpty();liveKind=s?.kind.orEmpty()}
