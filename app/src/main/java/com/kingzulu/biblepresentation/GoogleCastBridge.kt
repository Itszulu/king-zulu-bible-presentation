...
class GoogleCastBridge(context: Context) {
    ...
    fun publish(slide: PresentationSlide?, black: Boolean, theme: BackgroundTheme?, textSize: Int, autoFit: Boolean): Boolean {
        ...
        val payload = JSONObject().apply {
            put("type", "presentation_state"); put("bus", bus.name.lowercase()); put("black", black); put("reference", slide?.reference.orEmpty()); put("text", slide?.text.orEmpty()); put("translation", slide?.translation.orEmpty()); put("kind", slide?.kind.orEmpty()); put("textSize", textSize); put("autoFit", autoFit); put("overlay", theme?.overlay ?: .38f)
