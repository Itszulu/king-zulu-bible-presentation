...
object OfflineBibleRepository{
 ...
 fun initialize(context:Context){appContext=context.applicationContext;selected=context.getSharedPreferences("king_zulu_bibles",Context.MODE_PRIVATE).getString("selected","KJV")?:"KJV"}
 fun selectedTranslation():String=selected
 fun selectTranslation(abbreviation:String):Boolean{val key=abbreviation.uppercase();if(installedBibles().none{it.abbreviation.equals(key,true)})return false;selected=key;appContext?.getSharedPreferences("king_zulu_bibles",Context.MODE_PRIVATE)?.edit()?.putString("selected",key)?.apply();return true}
 ...
 /** Directory used by the future Translation Manager after a licensed module is downloaded. */
 fun moduleDirectory():File?=moduleDir()
 fun refreshModules(){bibleCache.keys.filter{!it.equals("KJV",true)}.forEach{bibleCache.remove(it)};indexCache.clear()}
