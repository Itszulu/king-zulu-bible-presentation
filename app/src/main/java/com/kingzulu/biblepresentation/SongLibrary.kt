...
data class SavedSong(val id:String,val title:String,val lyrics:String,val slides:List<LyricSlide>)
...
class SongLibrary(private val context:Context?=null){
    ...
    private fun load(){val c=context?:return;val raw=c.getSharedPreferences("king_zulu_songs",Context.MODE_PRIVATE).getString("library","[]")?:"[]";runCatching{val a=JSONArray(raw);for(i in 0 until a.length()){val o=a.getJSONObject(i);val lyrics=o.optString("lyrics");songs+=SavedSong(o.optString("id"),o.optString("title"),lyrics,LyricsSlideParser.parse(lyrics))}}}
}
